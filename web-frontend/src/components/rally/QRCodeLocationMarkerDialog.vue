<template>
  ...
  >
    ...
    >
      <v-card width="500px">
        <v-card-title>
          <span class="text-h5">QR Code Standort-Marker</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
...
const contentRules = [(v: string) => !!v || 'Der Inhalt des QR-Codes darf nicht leer sein.'];
...
async function submitForm(event: Event) {
  ...
  if (validationResult.valid) {
    ...
    qrCodeLocationMarker.value = new QRCodeLocationMarker('', 'QRCodeLocationMarker', ''); // reset data
  }
}
</script>
