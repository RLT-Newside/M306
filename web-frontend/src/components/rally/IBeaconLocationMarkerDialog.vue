<template>
  ...
  >
    ...
    >
      <v-card width="500px">
        <v-card-title>
          <span class="text-h5">iBeacon Standort-Marker</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
...
async function submitForm(event: Event) {
  ...
  if (validationResult.valid) {
    ...
    iBeaconLocationMarker.value = new IBeaconLocationMarker('', 'IBeaconLocationMarker', '', 0, 0); // reset data
  }
}
</script>
