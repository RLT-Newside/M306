<template>
  <v-container>
    ...
    <v-row>
      <v-col>
        <p class="body-text-1">
          RallyeId: <code>{{ rally?.id }}</code>
        </p>
        <p class="body-text-1">
          RallyeStageId: <code>{{ rallyStage?.id }}</code>
        </p>
        <p class="body-text-1">
...
function getAdditionalInformation(condition: Condition): string {
  if (condition.hasOwnProperty('locationId')) {
    ...
    const location = locations.find((loc) => loc.id == locationCondition.locationId);
...
async function saveCondition(): Promise<void> {
  ...
  if (payload != null) {
    try {
      await conditionApi.create(props.rallyId, props.rallyStageId, props.constraintId, payload);
      showSuccess('Bedingung wurde erstellt.');
      await loadConstraint(props.constraintId);
    } catch (error) {
      showError(error, {
        notFound: 'Die Bedingung konnte nicht erstellt werden, da ein Bezug fehlt.',
        forbidden: 'Keine Berechtigung zum Erstellen der Bedingung.',
        conflict: 'Die Bedingung ist ungültig oder bereits vorhanden.',
        fallback: 'Bedingung konnte nicht erstellt werden.',
      });
    }
  }
}

async function deleteCondition(condition: Condition): Promise<void> {
  try {
    await conditionApi.remove(props.rallyId, props.rallyStageId, props.constraintId, condition.id);
    showSuccess('Bedingung wurde gelöscht.');
    await loadConstraint(props.constraintId);
  } catch (error) {
    showError(error, {
      notFound: 'Die Bedingung existiert nicht mehr.',
      forbidden: 'Keine Berechtigung zum Löschen der Bedingung.',
      conflict: 'Die Bedingung kann aktuell nicht gelöscht werden.',
      fallback: 'Bedingung konnte nicht gelöscht werden.',
    });
  }
}
</script>
