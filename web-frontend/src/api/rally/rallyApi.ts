...
import type { Rally, RallyStage } from '../../models/rally/rally';
...
class RallyApi extends BaseApi {
  ...
  public getRallyStage(rallyId: string, rallyStageId: string): Promise<RallyStage> {
    return this.get<RallyStage>(`rally/v1/rally/${rallyId}/stage/${rallyStageId}`)
      .then<RallyStage>(RallyApi.then)
      .catch<RallyStage>(RallyApi.catch);
  }

  public addRallyStage(rallyId: string, rallyStage: RallyStage): Promise<RallyStage> {
    return this.post<RallyStage>(`rally/v1/rally/${rallyId}/stage`, JSON.stringify(rallyStage))
      .then<RallyStage>(RallyApi.then)
      .catch<RallyStage>(RallyApi.catch);
  }

  public deleteRallyStage(rallyId: string, rallyStageId: string): Promise<void> {
