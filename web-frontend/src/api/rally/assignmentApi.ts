...
class AssignmentApi extends BaseApi {
  ...
  public getAssignment(rallyId: string, assignmentId: string): Promise<Assignment> {
    return this.get<Assignment>(`rally/v1/rally/${rallyId}/assignment/${assignmentId}`)
      .then<Assignment>(AssignmentApi.then)
      .catch<Assignment>(AssignmentApi.catch);
  }

  public create(rallyId: string, assignment: Partial<Assignment>): Promise<Assignment> {
    return this.post<Assignment>(`rally/v1/rally/${rallyId}/assignment`, JSON.stringify(assignment))
      .then<Assignment>(AssignmentApi.then)
      .catch<Assignment>(AssignmentApi.catch);
  }

  public remove(rallyId: string, assignmentId: string): Promise<void> {
    return this.delete<void>(`rally/v1/rally/${rallyId}/assignment/${assignmentId}`)
