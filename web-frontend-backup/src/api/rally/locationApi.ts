...
class LocationApi extends BaseApi {
  ...
  public create(stageId: string, location: Partial<Location>): Promise<Location> {
    return this.post<Location>(`rally/v1/stage/${stageId}/location`, JSON.stringify(location))
      .then<Location>(LocationApi.then)
      .catch<Location>(LocationApi.catch);
  }

  public update(stageId: string, location: Location): Promise<Location> {
    return this.put<Location>(`rally/v1/stage/${stageId}/location`, JSON.stringify(location))
      .then<Location>(LocationApi.then)
      .catch<Location>(LocationApi.catch);
  }
