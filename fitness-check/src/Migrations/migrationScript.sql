...
BEGIN
    ...
    VALUES ('20241018194544_InitialMigration', '9.0.10');
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20241018202630_AttemptMoment', '9.0.10');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20251014155257_MultipleSchoolYears') THEN

    ALTER TABLE `TwelveMinutesRunAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    ALTER TABLE `StandingLongJumpAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    ALTER TABLE `ShuttleRunAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    ALTER TABLE `OneLegStandAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    ALTER TABLE `MedicineBallPushAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    ALTER TABLE `CoreStrengthAttempts` DROP COLUMN `Points`;
...
BEGIN
    ...
    CREATE TABLE `BestAttempts` (
        `Id` int NOT NULL AUTO_INCREMENT,
        `UserId` char(36) COLLATE ascii_general_ci NOT NULL,
        `CohortId` char(36) COLLATE ascii_general_ci NOT NULL,
        `Discipline` longtext CHARACTER SET utf8mb4 NOT NULL,
        `Result` float NOT NULL,
        `Points` int unsigned NOT NULL,
        `LeftFootResult` int unsigned NULL,
        `RightFootResult` int unsigned NULL,
        `LastCalculated` datetime(6) NOT NULL,
        CONSTRAINT `PK_BestAttempts` PRIMARY KEY (`Id`)
    ) CHARACTER SET=utf8mb4;
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20251014155257_MultipleSchoolYears', '9.0.10');
...
CREATE PROCEDURE MigrationsScript()
BEGIN
    IF NOT EXISTS(SELECT 1 FROM `__EFMigrationsHistory` WHERE `MigrationId` = '20251016064413_AddMomentForBestAttempt') THEN

    ALTER TABLE `BestAttempts` MODIFY COLUMN `Discipline` varchar(255) CHARACTER SET utf8mb4 NOT NULL;
...
BEGIN
    ...
    ALTER TABLE `BestAttempts` ADD `MomentUtc` datetime(6) NOT NULL DEFAULT '0001-01-01 00:00:00';
...
BEGIN
    ...
    CREATE UNIQUE INDEX `IX_BestAttempts_UserId_CohortId_Discipline` ON `BestAttempts` (`UserId`, `CohortId`, `Discipline`);
...
BEGIN
    ...
    INSERT INTO `__EFMigrationsHistory` (`MigrationId`, `ProductVersion`)
    VALUES ('20251016064413_AddMomentForBestAttempt', '9.0.10');
